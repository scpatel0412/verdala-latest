...
const Vision = ({ pageTitle, children }) => {
  const [slidecount, setSlidecount] = useState(0);
  const [slidecountSecound, setSlidecountSecound] = useState(0);
  const [navLinks, setNavLinks] = useState([]);
  const [slider3, setSliderSlider3] = useState({ nav1: null, nav2: null });
  const [slider4, setSliderSlider4] = useState({
    navSecound_1: null,
    navSecound_2: null,
  });
  let slider1 = useRef(null);
  let slider2 = useRef(null);
  let slider5 = useRef(null);
  let slider6 = useRef(null);
  function SampleNextArrow(props) {
    useEffect(() => {
      setSlidecount(props.slideCount);
    }, [props.slideCount]);
  ...
  function SampleNextSecoundArrow(props) {
    useEffect(() => {
      setSlidecountSecound(props.slideCount);
    }, [props.slideCount]);
    const { className, style, onClick } = props;
    return (
      <div
  ...
  function SamplePrevSecoundArrow(props) {
    const { className, style, onClick } = props;
    return (
      <div
        className={props.className}
  ...
  useEffect(() => {
    if (typeof data !== "undefined") {
      if (Object.keys(data).length > 0) {
        ...
        setNavLinks(removedata);
        setSliderSlider3({ nav1: slider1, nav2: slider2 });
        setSliderSlider4({ navSecound_1: slider5, navSecound_2: slider6 });
      }
    }
  }, [data]);
          ...
          <div className="vision-midd-body-content">
            <div className="then-now-sec" id="thenandnow">
              <div className="then-now-inner">
                ...
                <div className="then-now-inner-right anim-scroll-up">
                  <p>{`${
                    data?.then_and_now?.description.split("world")[0]
                  } world`}</p>
                  <p>{data?.then_and_now?.description.split("world")[1]}</p>
                </div>
                <div className="then-now-left-image-mobile anim-scroll-up">
            ...
            <TodayVision data={data} />
            <div className="then-now-sec" id="thenandnownew">
              <div className="then-now-inner">
                <div className="then-now-inner-left">
                  <div className="count-col">
                    <span className="count-number">03</span>
                    <p className="count-title anim-scroll-up">
                      {data?.then_and_now_new?.sub_title}
                    </p>
                  </div>
                  <div className="then-now-left-image">
                    <img
                      src={data?.then_and_now_new?.image?.url}
                      alt={data?.then_and_now_new?.image?.url}
                    />{" "}
                  </div>
                </div>
                <div className="then-now-inner-middle anim-scroll-up">
                  <h1>{data?.then_and_now_new?.title}</h1>
                </div>
                <div className="then-now-inner-right anim-scroll-up">
                  <p>{`${
                    data?.then_and_now_new?.description.split("world")[0]
                  } world`}</p>
                  <p>{data?.then_and_now_new?.description.split("world")[1]}</p>
                </div>
                <div className="then-now-left-image-mobile anim-scroll-up">
                  <img
                    src={data?.then_and_now_new?.image?.url}
              ...
              <div className="then-now-slider-Right">
                <div className="content-slider anim-scroll-up">
                  <Slider
                    asNavFor={slider4.navSecound_2}
                    fade={true}
                    ref={(slider) => (slider5 = slider)}
                    nextArrow={
                      <SampleNextSecoundArrow className="customarrow customarrownext" />
                    }
                    prevArrow={
                      <SamplePrevSecoundArrow className="customarrow customarrowprev" />
                    }
                  >
                    {data?.then_and_now_new?.slider_conent?.map((i, index) => {
                      return (
                        <div className="slider_content" key={index}>
                          <h3>{`${i.description}`}</h3>
                          <p>{i.author}</p>
                ...
                <div className="image-slider">
                  <Slider
                    // {...settings}
                    asNavFor={slider4.navSecound_1}
                    ref={(slider) => (slider6 = slider)}
                    slidesToShow={1}
                    swipeToSlide={true}
                    focusOnSelect={true}
                  >
